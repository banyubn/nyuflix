    ...
    <div class="h-screen flex items-center justify-center flex-column gap-5">
        <div>
            <div class="text-left mb-5">
                <h1 class="text-7xl">Edit Profile</h1>
                <p class="text-3xl text-secondary">Edit your profile for another person watching Nyuflix.</p>
            </div>
            <form action="{{ route('member.profile.update', $data->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="flex items-center justify-center gap-4">
                    ...
                    <input value="{{ $data->name }}" type="text" name="name" id="name" placeholder="Name"
                        class="w-full p-3 mt-1 outline-none text-gray-100 w-96 bg-gray-500/80 hover:bg-gray-500" required>
                </div>
                <div class="text-left mb-5">
                    <button type="submit"
                        class=" text-2xl outline outline-1 transition text-black bg-white w-25 hover:invert hover:text-white font-bold py-3 px-4 mt-5 mr-4">Continue</button>
